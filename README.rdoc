...
1. Modify the submodule's code at the same time you're iterating on the project
  that uses it - so especially useful if you want greater modularization of
...
2. Have your repository/branch "pointing" to a specific commit of the
  ...
  (which could be a local clone of any git repository whether you have "push"
  access or not) - and thus just by switching branches you're also changing the
  version of the submodules you're depending on.
